    ...
    for result in results:
        ...
        for box in boxes:
            ...
            label = model.names.get(
                cls, "unknown"
            )  # Évite les erreurs si l'ID est invalide
            conf = float(box.conf)
        ...
        while True:
            if not pause:
                ...
                if time.time() - start_time >= DETECTION_WINDOW:
                    if fonc.alert_level_gamma(detection_history):
                        alert_text = "Alerte Niveau Gamma"
                    elif fonc.alert_level_beta(detection_history):
                        alert_text = "Alerte Niveau Beta"
                    elif fonc.alert_level_alpha(detection_history):
                        alert_text = "Alerte Niveau Alpha"
                    elif fonc.alert_level_alpha_thermal(detection_history):
                        alert_text = "Alerte Niveau Alpha Thermal"    
                    else:
                        alert_text = None  # Pas d'alerte détectée
